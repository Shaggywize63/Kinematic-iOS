//
//  LeadPhotoSection.swift
//  Kinematic CRM
//
//  Reusable "Lead Photo (optional)" form section for the create / edit
//  sheets. Lets the user pick from camera or gallery, uploads to
//  /api/v1/upload/photo via CRMService, and stores the resulting URL in
//  the parent's `photoUrl` binding.
//
...
struct LeadPhotoSection: View {
    ...
    private func upload(_ image: UIImage) async {
        ...
        do {
            let url = try await CRMService.shared.uploadLeadPhoto(image)
            photoUrl = url
        } catch {
