    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let addr = account.billingAddress {
                    detailRow("Billing", addr, icon: "mappin.and.ellipse", color: .red)
                }
                if let phone = account.phone {
                    detailRow("Phone", phone, icon: "phone.fill", color: .green)
                }
                if let site = account.website {
                    detailRow("Website", site, icon: "globe", color: .blue)
                }
            }
            .padding()
        }
