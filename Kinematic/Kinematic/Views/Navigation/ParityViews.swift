...
struct ProfileView: View {
    ...
    // Re-read on each body evaluation via the `user` computed property so
    // the avatar/name refresh after AvatarPicker writes back to
    // Session.currentUser. The previous stored-property capture cached the
    // pre-upload value and the image never appeared until app restart.
    private var user: User? { Session.currentUser }

    /// Bumped whenever the avatar changes so the entire view tree recomputes.
    /// `Session.currentUser` lives in UserDefaults — there's no @Published
    /// signal we can subscribe to, so we drive the refresh by tweaking
    /// local state when the AvatarPicker reports success.
    @State private var avatarRefreshTick: Int = 0
    ...
    var body: some View {
        ScrollView {
            ...
            .padding(.top, 20)
            .id(avatarRefreshTick) // force-rebuild after avatar mutation
        }
        ...
        VStack(spacing: 20) {
            Button {
                ...
                ZStack(alignment: .bottomTrailing) {
                    ...
                    // Camera affordance so the tap target reads as editable.
                    Circle()
