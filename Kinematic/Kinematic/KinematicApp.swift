...
struct User: Codable, Identifiable {
    ...
    let mobile: String?
    let orgId: String?
    /// Signed Supabase URL for the user's avatar. `nil` when the user
    /// hasn't uploaded one — UI falls back to the initial-only chip.
    /// Populated by `GET /api/v1/auth/me` and mutated through
    ...
    /// Return a copy with `avatarUrl` replaced. Used after the
    /// PATCH /auth/me response so the rest of the app can refresh
    /// without rebuilding the whole user blob client-side.
    func withAvatarUrl(_ newValue: String?) -> User {
        User(
            id: id,
            name: name,
            role: role,
